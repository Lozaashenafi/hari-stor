    ...
    <div className="space-y-10 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard 
          ...
          icon={<ShoppingBag className="text-[#C5A059]" size={20} />} 
          subtitle="Unique hair types"
        />
        <StatCard 
          title="In Hand Inventory" 
          value={stats.inHandCount} 
          icon={<Box className="text-[#C5A059]" size={20} />} 
          subtitle="Ready for immediate ship"
        />
        <StatCard 
          title="Vault Value" 
          value={`$${stats.inventoryValue.toLocaleString()}`} 
          icon={<DollarSign className="text-[#C5A059]" size={20} />} 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-12">
        <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
          <div className="relative z-10">
            ...
            <Link href="/admin/products" className="inline-flex items-center gap-3 text-[#C5A059] text-[10px] uppercase tracking-widest font-black group-hover:gap-5 transition-all">
        ...
        <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
          <div className="relative z-10">
            ...
            <Link href="/admin/profile" className="inline-flex items-center gap-3 text-[#C5A059] text-[10px] uppercase tracking-widest font-black group-hover:gap-5 transition-all">
...
function StatCard({ title, value, icon, subtitle }: any) {
  return (
    <div className="bg-zinc-900 border border-zinc-800 p-8 rounded-[2rem] shadow-xl hover:border-[#C5A059]/30 transition-all group">
      <div className="flex items-center justify-between mb-6">
        <div className="p-3 bg-black rounded-2xl border border-zinc-800 group-hover:border-[#C5A059]/50 transition-colors">
          {icon}
        </div>
